...
`sync` orchestrates both phases end to end. `audible` and `covers` are the two capabilities
that still work directly against rendered notes (audible enrich-only via `VaultIndex.find`;
covers scans `type: book` notes) — they run after `render`. `VaultIndex.find_or_create` is
no longer used by any importer; note creation belongs solely to `render`.
...
- **Two ways to resolve a book to a note** (`VaultIndex`): `find_or_create(ref)` matches an
  existing note or creates a flat stub for a new book; `find(ref)` is match-only (returns the
  existing `BookNote` or `None`, never creating a file). Both match by ISBN, then Amazon id,
  then standardized (title, author). In the current CSV-store pipeline the source importers no
  longer use `VaultIndex` at all — they write to the store and book identity is resolved by
  `store.Catalog.find` (same ISBN → Amazon → title/author precedence). `VaultIndex.find` is
  still used by the note-based `audible` capability to enrich an already-rendered note;
  `find_or_create` has no remaining caller (note creation belongs to `render`).
