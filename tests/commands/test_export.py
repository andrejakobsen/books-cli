...
def test_book_frontmatter_no_cover_when_file_missing(tmp_path):
    ...
    note = tmp_path / "Books" / "X - A.md"
    row = store.BookRow(
        book_id="X - A", title="X", authors=["A"], cover="Data/Sources/_covers/calibre/0.jpg"
    )
    meta = R.book_frontmatter(row, note, existing={}, has_highlights=False)
...
def test_render_command_renders_vault(tmp_path):
    ...
    store.merge(vault)
    result = CliRunner().invoke(app, ["export", "--output", str(vault)])
    assert result.exit_code == 0, result.output
    assert (vault / "Books" / "X - A.md").is_file()


def test_render_command_errors_without_books_csv(tmp_path):
    result = CliRunner().invoke(app, ["export", "--output", str(tmp_path / "vault")])
...
def test_render_command_rejects_removed_no_obsidian_flag(tmp_path):
    ...
    store.write_layer(vault, "calibre", [store.BookRow(title="X", authors=["A"], format="ebook")])
    store.merge(vault)
    result = CliRunner().invoke(app, ["export", "--no-obsidian", "--output", str(vault)])
    assert result.exit_code != 0
...
def test_render_command_refresh_deletes_stale_note(tmp_path):
    ...
    stale.write_text("---\ntype: book\ntitle: Gone\n---\n", encoding="utf-8")
    result = CliRunner().invoke(app, ["export", "--refresh", "--output", str(vault)])
    assert result.exit_code == 0, result.output
...
def test_book_frontmatter_preserves_aliases_and_cssclasses(tmp_path):
    note = tmp_path / "Books" / "X - A.md"
    row = store.BookRow(book_id="X - A", title="X", authors=["A"])
    existing = {"topics": ["[[History]]"], "aliases": ["The X Book"], "cssclasses": ["book"]}
...
def test_render_materializes_staged_cover(tmp_path):
    ...
    note_text = (vault / "Books" / "The Deluge - Adam Tooze.md").read_text(encoding="utf-8")
