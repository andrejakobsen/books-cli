...
def test_strip_series_extracts_clean_title():
    assert (
        rw.strip_series("Stalin: Volume I: Paradoxes of Power, 1878-1928 (Stalin #1)")
        == "Stalin: Volume I: Paradoxes of Power, 1878-1928"
    )


def test_strip_series_decimal_index():
    assert rw.strip_series("Some Book (Saga #2.5)") == "Some Book"


def test_strip_series_no_suffix_is_verbatim():
    assert rw.strip_series("The Landscape of History") == "The Landscape of History"


def test_strip_series_ignores_non_numbered_parenthetical():
    # A trailing parenthetical without "#N" must NOT be treated as a series.
    title = "The Landscape of History: How Historians Map the Past (Inaugural Lectures)"
    assert rw.strip_series(title) == title
...
def test_convert_same_title_different_authors_no_amazon_stay_separate(tmp_path):
    ...
    csv = tmp_path / "rw.csv"
    csv.write_text(
        HEADER + '"From A.","Selected Essays",Author A,,,,,page,1,2026-01-01 00:00:00+00:00,\n'
    ...
    seed_books(
        ...
        [
            store.BookRow(
                book_id="Selected Essays - Author A", title="Selected Essays", authors=["Author A"]
            ),
            store.BookRow(
                book_id="Selected Essays - Author B", title="Selected Essays", authors=["Author B"]
            ),
        ],
    )
...
def test_convert_same_amazon_different_title_rows_group_together(tmp_path):
    ...
    seed_books(
        ...
        [
            store.BookRow(
                book_id="Book - Kotkin", title="Book", authors=["Kotkin"], amazon="B00INIXPYE"
            )
        ],
    )
    stats = rw.convert(csv, vault)
...
def test_readwise_command_end_to_end(tmp_path):
    ...
    vault = tmp_path / "Vault"
    seed_stalin(vault)
    result = CliRunner().invoke(app, ["--csv", str(write_csv(tmp_path)), "--output", str(vault)])
